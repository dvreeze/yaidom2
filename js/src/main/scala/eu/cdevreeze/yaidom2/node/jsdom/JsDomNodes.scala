...
object JsDomNodes {
  ...
  object Elem extends BackingElemFunctionsApi {
    ...
    def filterAncestorElems(elem: ElemType, p: ElemType => Boolean): Seq[ElemType] = {
      findParentElem(elem).to(ArraySeq).flatMap(e => filterAncestorElemsOrSelf(e, p))
    }
    ...
    def filterAncestorElemsOrSelf(elem: ElemType, p: ElemType => Boolean): Seq[ElemType] = {
      // Recursive calls
      ArraySeq(elem).filter(p) ++ findParentElem(elem).to(ArraySeq).flatMap(e => filterAncestorElemsOrSelf(e, p))
    }
